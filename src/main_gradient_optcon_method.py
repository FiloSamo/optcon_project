...
dt = 0.001   # get discretization step from dynamics
ns = dyn.ns
ni = dyn.ni
...
xx_ref, uu_ref = ref_gen.gen(tf=tf,dt=dt,ns=ns,ni=ni)
...
_, AA_ref,BB_ref = dyn.dynamics(xx_ref[:,TT-1], uu_ref[:,TT-1], dt)

QQT,_ ,_ = ctrl.dare(AA_ref,BB_ref, QQt, RRt)
...
for kk in range(max_iters-1):
  ...
  for tt in range(TT-1):
    temp_cost,_ ,_ = cst.stagecost(xx[:,tt, kk], uu[:,tt,kk], xx_ref[:,tt], uu_ref[:,tt])
    JJ[kk] += temp_cost
  
  temp_cost,_ = cst.termcost(xx[:,-1,kk], xx_ref[:,-1],QQT)
  JJ[kk] += temp_cost
  ...
  lmbd_temp,_= cst.termcost(xx[:,TT-1,kk], xx_ref[:,TT-1], QQT)

  lmbd[:,TT-1,kk] = lmbd_temp.copy().squeeze()

  for tt in reversed(range(TT-1)):  # integration backward in time

    _, qt, rt = cst.stagecost(xx[:,tt, kk], uu[:,tt,kk], xx_ref[:,tt], uu_ref[:,tt])
    _, At, Bt = dyn.dynamics(xx[:,tt,kk], uu[:,tt,kk], dt)
  ...
  for tt in range(TT-1):
    ...
    z,_ ,_ = dyn.dynamics(xx_temp[:,tt], uu_temp[:,tt], dt)
    xx_temp[:,tt+1] = z.squeeze()
